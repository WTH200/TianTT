...
def getinfo(link):
    try:
        ...
        return False
def setstatus():
    u='http://175.24.153.42:8881/setstatus'
    p={'key':key,'type':'clks','val':'1'}
    r=requests.get(u,params=p)
    print(r.text)

def getstatus():
    u = 'http://175.24.153.42:8881/getstatus'
    p = {'key':key,'type':'clks'}
    r = requests.get(u, params=p)
    return r.text
...
if __name__ == '__main__':
    ...
    mycklist=[
        {'authtoken': 'eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzUxMiJ9.eyJzdWIiOiIyMDIzMDgyODAxMzUzNTU4NTciLCJpYXQiOjE2OTMyMDIwMzUsImV4cCI6MTY5MzQ2MTIzNX0.GQZ1HApc1t5YiY02fwa0MMzpAZrBUUQsYe0nGAi3KrwKTJc_cEUl1At7ZtUpklIIiRgWmGXhP6p0vehqyZnDTw; snapshot=0'}
    ]
    for i in mycklist:
        api = WXYD(i)
        api.run()
