 ...
 * 项目名称：番茄免费小说
 * 项目更新：2023-08-27
 * 项目抓包：抓api5-normal-lq.fqnovel.com下的宝箱url#cookie#x-argus#x-ladon#user-agent填入变量
 * 开启抓包，点击宝箱获得金币，点击弹出的看广告，看完一段广告，搜索/luckycat/novel/v1/task/done/excitation_ad
 * 项目抓包：复制/luckycat/novel/v1/task/done/excitation_ad?后面的值，后面的值#cookie#x-argus#x-ladon#user-agent
 * 项目变量：lekebo_fqmfxs_Cookie
 * 项目定时：每10分钟运行一次
 * 定时规则: * 10 0-23 * * *
 * 
 * 版本功能: 签到、宝箱、广告，后期会完善待增加的功能
 * 
 * 更新内容: 第一版功能暂时不多，还有就是要等群友上线才可以抓基础包所以得慢慢更新
 * 
...
let userCookie = ($.isNode() ? process.env.lekebo_fqmfxs_Cookie : $.getdata('lekebo_fqmfxs_Cookie')) || '';
let userList = [];
let userIdx = 0;
...
let host = 'api5-normal-lq.fqnovel.com';
...
// ============================================执行项目============================================ \\
    async getMemberInfo(timeout = 2000) {
        return new Promise((resolve) => {
            let url = {
                url: `${hostname}/reading/user/info/v/?${this.ck[0]}`,
                headers: {
                    Host: host,
                ...
                try {
                    if (error) {
                        ...
                        if (result == true) {
                            ...
                            if (result.code == 0) {
                                DoubleLog(`\n ✅ 【${this.index}】用户信息: ${result.data.user_name}`)
                            } else {
    ...
    async getMemberScore(timeout = 2000) {
        return new Promise((resolve) => {
            let url = {
                url: `${hostname}/luckycat/novel/v1/task/page?${this.ck[0]}`,
                headers: {
                    Host: host,
                    'Connection': 'keep-alive',
				    'Content-Type': 'application/json; charset=utf-8',
    ...
    async invitation(timeout = 2000) {
        return new Promise((resolve) => {
            let url = {
                url: `${hostname}/luckycat/novel/v1/task/done/post_invite_code?${this.ck[0]}`,
                headers: {
                    Host: host,
                    'Connection': 'keep-alive',
				    'Content-Type': 'application/json; charset=utf-8',
    ...
    async signin(timeout = 2000) {
        return new Promise((resolve) => {
            let url = {
                url: `${hostname}/luckycat/novel/v2/task/page?${this.ck[0]}`,
                headers: {
                    Host: host,
				    'Connection': 'keep-alive',
				    'Content-Type': 'application/json; charset=utf-8',
    ...
    async opensign(timeout = 2000) {
        return new Promise((resolve) => {
            let url = {
                url: `${hostname}/luckycat/novel/v1/task/done/sign_in?${this.ck[0]}`,
                headers: {
                    Host: host,
				    'Connection': 'keep-alive',
				    'Content-Type': 'application/json; charset=utf-8',
    ...
    async open_sign_ad(timeout = 2000) {
        return new Promise((resolve) => {
            ...
            let url = {
                url: `${hostname}/luckycat/novel/v1/task/sign_in/detail?${this.ck[0]}`,
                headers: {
                    Host: host,
				    'Connection': 'keep-alive',
				    'Content-Type': 'application/json; charset=utf-8',
    ...
    async signin_video(adid,reqid,amount,opentimes,timeout = 2000) {
        return new Promise((resolve) => {
            let url = {
                url: `${hostname}/luckycat/novel/v1/eat/done_eat/${this.ck[0]}`,
                headers: {
                    Host: host,
				    'Connection': 'keep-alive',
				    'Content-Type': 'application/json; charset=utf-8',
    ...
    async open_treasure(timeout = 2000) {
        return new Promise((resolve) => {
            let url = {
                url: `${hostname}/luckycat/novel/v1/task/done/treasure_task?${this.ck[0]}`,
                headers: {
                    Host: host,
                    'Connection': 'keep-alive',
				    'Content-Type': 'application/json; charset=utf-8',
    ...
    async treasure_video(timeout = 2000) {
        return new Promise((resolve) => {
            let url = {
                url: `${hostname}/luckycat/novel/v1/task/done/excitation_ad_treasure_box?${this.ck[0]}`,
                headers: {
                    Host: host,
                    'Connection': 'keep-alive',
				    'Content-Type': 'application/json; charset=utf-8',
                ...
                body: `{"from":"reward_dialog_from_treasure_box","position":"task_page","task_key":"excitation_ad_treasure_box"}`,
            }
            $.post(url, async (error, response, data) => {
                try {
                    if (error) {
                                ...
                                for(let i=0; i < result.data.task_list_v2.length; i++) {
                                    ...
                                    if (taskItem.task_id == "111") {
                                        if (taskItem.completed == true) {
                                            ...
                                            if (tasklist.end_time != null) {
                                                DoubleLog(`\n ❌ 【${this.index}】广告赚金: 下次广告：${$.time('yyyy-MM-dd HH:mm:ss', next_time * 1000)} 分钟`);
                                            } else {
    ...
    async open_video(timeout = 2000) {
        return new Promise((resolve) => {
            let url = {
                url: `${hostname}/luckycat/novel/v1/task/done/excitation_ad?${this.ck[0]}`,
                headers: {
                    Host: host,
                    'Connection': 'keep-alive',
				    'Content-Type': 'application/json; charset=utf-8',
                                    ...
                                    if (taskItem.task_id == "1") {
                                        ...
                                        if (taskItem.completed == true) {
                                            ...
                                            if (timeHours > 4 && timeHours < 9) {
                                                ...
                                                DoubleLog(`\n ❌ 【${this.index}】吃饭打卡: 下次打卡：${$.time('yyyy-MM-dd HH:mm:ss', taskItem.end_time)}`);
                                            }
                                        }
                                    }
                                }
                ...
                try {
                    if (error) {
                        ...
                        if (result.err_no == 0) {
                            ...
                            DoubleLog(`\n ❌ 【${this.index}】吃饭奖励: ${result.message}`)
                        }
                    }
                } catch (e) {
                    $.logErr(e, response);
